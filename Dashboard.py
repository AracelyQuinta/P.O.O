...
def mostrar_codigo(ruta_script):
    ...
    try:
        with open(ruta_script_absoluta, 'r') as archivo:  # Abre el archivo en modo lectura
            codigo = archivo.read()  # Lee el contenido completo del archivo
            print(f"\n--- Código de {ruta_script} ---\n")  # Imprime el nombre del archivo
            print(codigo)  # Imprime el contenido del archivo
            return codigo  # Devuelve el contenido leído
    except FileNotFoundError:  # Si no se encuentra el archivo
        print("El archivo no se encontró.")
        return None
    except Exception as e:  # Cualquier otro error durante la lectura
...
def ejecutar_codigo(ruta_script):
    try:
        if os.name == 'nt':  # Si se está ejecutando en Windows
            subprocess.Popen(['cmd', '/k', 'python', ruta_script])  # Usa cmd para ejecutar el script con Python
        else:  # Si se está en Linux o macOS
            subprocess.Popen(['xterm', '-hold', '-e', 'python3', ruta_script])  # Usa xterm para ejecutar y mantener abierta la terminal
    except Exception as e:  # Si hay algún error en la ejecución
...
def mostrar_menu():
    ...
    while True:  # Bucle para mantener el menú activo
        ...
        for key in unidades:  # Imprime cada unidad disponible
            print(f"{key} - {unidades[key]}")
        print("0 - Salir")  # Opción para salir

        eleccion_unidad = input("Elige una unidad o '0' para salir: ")  # Entrada del usuario
        if eleccion_unidad == '0':  # Si elige salir
...
def mostrar_sub_menu(ruta_unidad):
    ...
    while True:
        ...
        eleccion_carpeta = input("Elige una subcarpeta o '0' para regresar: ")  # Entrada del usuario
        if eleccion_carpeta == '0':  # Regresa si elige 0
            break
        else:
            try:
                eleccion_carpeta = int(eleccion_carpeta) - 1  # Convierte la elección en índice
                if 0 <= eleccion_carpeta < len(sub_carpetas):  # Si el índice es válido
                    mostrar_scripts(os.path.join(ruta_unidad, sub_carpetas[eleccion_carpeta]))  # Muestra los scripts de la carpeta elegida
                else:
        ...
        if eleccion_script == '0':  # Regresa al submenú
            ...
            try:
                ...
                if 0 <= eleccion_script < len(scripts):  # Verifica que el índice sea válido
                    ...
                    if codigo:  # Si se pudo leer el código
                        ejecutar = input("¿Desea ejecutar el script? (1: Sí, 0: No): ")  # Pregunta si desea ejecutar
                        if ejecutar == '1':  # Ejecuta si elige 1
                            ejecutar_codigo(ruta_script)
                        elif ejecutar == '0':  # No ejecuta si elige 0
                            print("No se ejecutó el script.")
                        else:  # Cualquier otra opción
                            print("Opción no válida. Regresando al menú de scripts.")
                        input("\nPresiona Enter para volver al menú de scripts.")  # Pausa antes de volver al menú
                else:
                    print("Opción no válida. Por favor, intenta de nuevo.")  # Si el índice está fuera del rango
            except ValueError:  # Si la entrada no es un número
                print("Opción no válida. Por favor, intenta de nuevo.")
